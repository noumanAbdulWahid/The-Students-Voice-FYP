...
const Profile = () => {
  ...
  const {user: {_id, name, profileImg, imageUrl, userID , session, currentSemester, department}} = useSelector((state)=> state.AuthReducers);
  

  const {} = useQuery('currentUser', async ()=>{
    await axios.get( `/current-user/${_id}`, { cache: 'default' })
    .then(response => {
    ...
    <div>
      <div className="header w-11 mx-auto flex sm:flex-column md:flex-row lg:flex-row xl:flex-row" style={{flexDirection:'column'}}>
        ...
        <div className='md:mr-0 lg:mr-0 xl:mr-0 md:w-3 lg:w-3 xl:w-2' style={{marginLeft: 'auto', marginRight: 'auto', width: '60%', marginTop:'10px'}}>
          <EditProfile currentUserEducation={currentUserEducation} currentUserAbout = {currentUserAbout} currentUserJob= {currentUserJob} />
        </div>
      </div>
